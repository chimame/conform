...
export async function action({ request }: Route.ActionArgs) {
	...
	const result = await standardValidate(schema, submission.value);

	if (result.issues) {
		return {
			result: report(submission, {
				error: resolveStandardSchemaResult(result),
			}),
		};
	}

	throw redirect(`/?value=${JSON.stringify(result.value)}`);
}
...
export default function Login({ actionData }: Route.ComponentProps) {
	const { form, fields } = useForm({
		...
		onValidate: async (value) => {
			return resolveStandardSchemaResult(await standardValidate(schema, value));
		},
	});
